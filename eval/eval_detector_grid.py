...
def get_y_score(link_list, conf, y_len):
    y_score = np.zeros(y_len)
    y_true = np.ones(y_len)
...
def mean_average_precision(pred_center, lbl_center, conf, link_r=None, eval_range=(5, 10), step=0.5, avg=False):
    if link_r is not None:
        ...
        for link_r in np.arange(eval_range[0], eval_range[1], step):
            if len(pred_center) != 0:
                ...
                map, mrec, mprec = voc_ap(list(rec[::-1]), list(prec[::-1]))
                # tp, n_recall, n_precision = eval_utils.grid_score(lbl_center, pred_center, link_list)
                # link_d.append(link_r)
                precision.append(mprec)
                recall.append(mrec)
            else:
                precision.append(0)
                recall.append(0)
...
def eval_model(model_name, city_name, tile_id, visualize=False, link_r=None, is_box=False):
    ...
    if not is_box:
        if link_r is None:
            precision, recall = mean_average_precision(pred_center, lbl_center, conf)
            return precision, recall
        else:
            tp, n_recall, n_precision = mean_average_precision(pred_center, lbl_center, conf, link_r)
            return tp, n_recall, n_precision
    else:
        if link_r is None:
...
if __name__ == '__main__':
    ...
    """
    tower_results('inception', link_r=10, is_box=False)
    tower_results('res101', link_r=10, is_box=False)
    tower_results('res50', link_r=10, is_box=False)
